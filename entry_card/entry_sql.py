...
def get_data():
    ...
    if accept_var.get():
        ...
        cursor = conn.cursor()
        table_create_query = '''CREATE TABLE IF NOT EXISTS students (first_name TEXT, 
                last_name TEXT, gender TEXT, military TEXT, student_id INTEGER, grade TEXT, subgroup INTEGER)'''
        cursor.execute(table_create_query)
        data_insert_query = '''INSERT INTO students (first_name, last_name, gender, military, student_id, grade, subgroup)
                VALUES (?, ?, ?, ?, ?, ?, ?)'''
        data_insert_tuple = ([*data.values()])
